...
//	}
	...
	@PostMapping(value = "/district/search") //add param va gui post len api va tra ve kieu chuoi
	@HystrixCommand(fallbackMethod = "fallBackMethod")
	public @ResponseBody String callFindAllProvince(@RequestBody Map<String,Object> body) {
		...
		String pre_id 	= body.get("preId").toString();
		String dit_name = body.get("ditName").toString();
	    ...
	    params.add(new BasicNameValuePair("pre_id", pre_id));
	    params.add(new BasicNameValuePair("dit_name", dit_name));
	    
	    System.out.println(serviceInstance.getUri().toString());
	    String data;
	    data=  getAPIData.getDataURL(serviceInstance.getUri().toString()+"/api/district", params);
		return data;
	} 
